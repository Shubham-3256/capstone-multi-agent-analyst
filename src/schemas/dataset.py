...
class ColumnInfo(BaseModel):
    ...
    name: str = Field(..., description="Name of the target column", examples=["age"])
    dtype: str = Field(
        ..., description="Data type inferred for the column values", examples=["int64"]
    )
    non_null_count: int = Field(
        ..., description="Count of non-null records in the column", examples=[1000]
    )
    null_count: int = Field(
        ..., description="Count of null records in the column", examples=[0]
    )
    ...
    unique_count: int = Field(
        ..., description="Number of distinct unique values in the column", examples=[45]
    )
...
class TargetInfo(BaseModel):
    ...
    column_name: str = Field(
        ..., description="Name of the selected target label column", examples=["target"]
    )
    task_type: str = Field(
        ...,
...
class DatasetSummary(BaseModel):
    ...
    filename: str = Field(
        ..., description="Sanitized name of the dataset file", examples=["dataset.csv"]
    )
    row_count: int = Field(
        ..., description="Total number of rows in the dataset", examples=[1000]
    )
    column_count: int = Field(
        ..., description="Total number of columns in the dataset", examples=[10]
    )
    file_size_bytes: int = Field(
        ..., description="Physical file size in bytes", examples=[1048576]
    )
    ...
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="Profiling evaluation timestamp"
    )
