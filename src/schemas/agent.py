...
class AgentResponse(BaseModel):
    ...
    thoughts: list[str] | None = Field(
        ...
        examples=[
            [
                "Detected 12 nulls in column 'age'.",
                "Inferred 'age' column data-type is float.",
                "Determined median age is 38.5.",
            ]
        ],
    )
    execution_time_seconds: float = Field(
        ..., description="Time taken to run the task in seconds", examples=[2.41]
    )
