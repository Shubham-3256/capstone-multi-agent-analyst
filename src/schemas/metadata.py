...
class DatasetMetadata(BaseModel):
    ...
    filename: str = Field(
        ..., description="Sanitized dataset filename", examples=["boston_housing.csv"]
    )
    filepath: str = Field(
        ...,
    ...
    row_count: int = Field(
        ..., description="Total row records in the dataset table", examples=[506]
    )
    column_count: int = Field(
        ..., description="Total columns in the dataset table", examples=[14]
    )
    ...
    duplicate_rows_count: int = Field(
        ..., description="Total count of duplicate rows in the dataset", examples=[0]
    )
    memory_usage_bytes: int = Field(
        ..., description="Estimated pandas RAM footprint in bytes", examples=[56800]
    )
    status: str = Field(
        default="uploaded",
