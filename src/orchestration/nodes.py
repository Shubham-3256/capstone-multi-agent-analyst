...
class WorkflowNodes:
    ...
    def visualization(self, state: WorkflowState) -> Dict[str, Any]:
        result = self.executor.run("visualization", state, lambda item: self.callbacks["visualization"](_get(item.data_intelligence_result, "cleaned_filepath"), item.feature_result, item.ml_result))
