...
class WorkflowNodes:
    ...
    def __init__(self, executor: NodeExecutor, callbacks: dict[str, Callable[..., Any]]) -> None:
    ...
    def load_dataset(self, state: WorkflowState) -> dict[str, Any]:
        ...
        result = self.executor.run("load_dataset", state, lambda _: DataLoader.load_file(Path(state.dataset_path)))
        return {"dataframe": None}

    def data_intelligence(self, state: WorkflowState) -> dict[str, Any]:
        result = self.executor.run("data_intelligence", state, lambda item: self.callbacks["data_intelligence"](Path(item.dataset_path), item.metadata.target_column))
    ...
    def feature_engineering(self, state: WorkflowState) -> dict[str, Any]:
        def invoke(item: WorkflowState) -> Any:
            ...
            return self.callbacks["feature_engineering"](DataLoader.load_file(Path(source)), item.metadata.target_column)
    ...
    def machine_learning(self, state: WorkflowState) -> dict[str, Any]:
        def invoke(item: WorkflowState) -> Any:
            ...
            return self.callbacks["machine_learning"](train.drop(columns=[target]), train[target], valid.drop(columns=[target]), valid[target])
    ...
    def visualization(self, state: WorkflowState) -> dict[str, Any]:
        result = self.executor.run("visualization", state, lambda item: self.callbacks["visualization"](_get(item.data_intelligence_result, "cleaned_filepath"), item.feature_result, item.ml_result, item.metadata.target_column))
        return {"visualization_result": result} if result is not None else {}

    def business_insights(self, state: WorkflowState) -> dict[str, Any]:
        result = self.executor.run("business_insights", state, lambda item: self.callbacks["business_insights"](item.dataset_profile, item.feature_result, item.ml_result, item.visualization_result))
        return {"business_result": result} if result is not None else {}

    def report_generation(self, state: WorkflowState) -> dict[str, Any]:
        result = self.executor.run("report_generation", state, lambda item: self.callbacks["report_generation"](item.dataset_profile, item.feature_result, item.ml_result, item.visualization_result, item.business_result, item.metadata.template_type))
