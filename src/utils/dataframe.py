...
def get_missing_summary(df: pd.DataFrame) -> dict:
    ...
    for col, val in missing_series.items():
        ...
        col_missing[str(col)] = {"count": val_int, "percentage": round(pct, 2)}

    return {"total_missing": total_missing, "column_missing": col_missing}
...
def extract_column_statistics(df: pd.DataFrame) -> dict[str, dict[str, Any]]:
    ...
    for col in df.columns:
        ...
        if pd.api.types.is_numeric_dtype(series):
            ...
            col_stats = {
                "mean": (
                    float(desc.get("mean", 0.0))
                    if not pd.isna(desc.get("mean"))
                    else 0.0
                ),
                "std": (
                    float(desc.get("std", 0.0)) if not pd.isna(desc.get("std")) else 0.0
                ),
                "min": (
                    float(desc.get("min", 0.0)) if not pd.isna(desc.get("min")) else 0.0
                ),
                "25%": (
                    float(desc.get("25%", 0.0)) if not pd.isna(desc.get("25%")) else 0.0
                ),
                "50%": (
                    float(desc.get("50%", 0.0)) if not pd.isna(desc.get("50%")) else 0.0
                ),
                "75%": (
                    float(desc.get("75%", 0.0)) if not pd.isna(desc.get("75%")) else 0.0
                ),
                "max": (
                    float(desc.get("max", 0.0)) if not pd.isna(desc.get("max")) else 0.0
                ),
            }
            ...
            col_stats = {
                ...
                "freq": (
                    int(desc.get("freq", 0)) if not pd.isna(desc.get("freq")) else 0
                ),
            }
