...
class DatasetVisualizer:
    ...
    def generate_missing_heatmap(
        df: pd.DataFrame, theme_name: str = "corporate"
    ) -> dict[str, Any]:
    ...
    def generate_correlation_heatmap(
        ...
        if numeric_df.empty or numeric_df.shape[1] < 2:
            logger.warning(
                "Insufficient numeric variables for correlation matrix. Creating dummy empty plots."
            )
        ...
        sns.heatmap(
            corr, annot=True, cmap="coolwarm", vmin=-1.0, vmax=1.0, square=True, ax=ax
        )
        fig_mpl.tight_layout()
    ...
    def generate_distribution_plot(
        df: pd.DataFrame, column_name: str, theme_name: str = "corporate"
    ) -> dict[str, Any]:
        ...
        if col not in df.columns:
            logger.warning(
                f"Column '{col}' not found. Selecting first column '{df.columns[0]}'."
            )
            col = df.columns[0]
        ...
        if is_numeric:
            ...
            fig_plotly.add_trace(go.Histogram(x=series.values, name=col))
        else:
            ...
            fig_plotly.add_trace(
                go.Bar(x=list(counts.index), y=list(counts.values), name=col)
            )
