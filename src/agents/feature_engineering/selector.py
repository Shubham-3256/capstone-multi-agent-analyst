...
class FeatureSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series | None = None) -> "FeatureSelector":
        ...
        """
        logger.info(
            f"FeatureSelector: Fitting feature selector (method={self.method.upper()}) on X shape {X.shape}"
        )
        ...
        if self.method in ["variance", "correlation", "mutual_info"]:
            self.variance_selector_ = VarianceThreshold(
                threshold=self.variance_threshold
            )
            self.variance_selector_.fit(numeric_df)
            support = self.variance_selector_.get_support()
            low_var_cols = [
                c for i, c in enumerate(numeric_df.columns) if not support[i]
            ]
            current_cols = [c for c in current_cols if c not in low_var_cols]
            logger.info(
                f"  Variance Threshold removed {len(low_var_cols)} features: {low_var_cols}"
            )
        ...
        if self.method in ["correlation", "mutual_info"] and len(current_cols) > 1:
            ...
            if not numeric_subset.empty:
                ...
                logger.info(
                    f"  Correlation Filter dropped {len(to_drop)} features: {to_drop}"
                )
        ...
        if self.method == "mutual_info" and y is not None:
            ...
            if not numeric_subset.empty:
                ...
                y_clean = pd.Series(y).fillna(
                    pd.Series(y).mode().iloc[0] if not pd.Series(y).mode().empty else 0
                )
                ...
                k_best_cols = [
                    col for i, col in enumerate(numeric_subset.columns) if support[i]
                ]
                ...
                logger.info(
                    f"  Mutual Info K-Best selected {len(k_best_cols)} numeric features: {k_best_cols}"
                )

        self.columns_to_keep_ = current_cols
        logger.info(
            f"FeatureSelector: Selected {len(self.columns_to_keep_)} / {X.shape[1]} features."
        )
        return self
