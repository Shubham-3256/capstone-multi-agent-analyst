...
class FeatureInfo(BaseModel):
    ...
    inferred_type: str = Field(..., description="Detected type ('numeric', 'categorical', 'ordinal', 'boolean', 'datetime', 'identifier', 'text')", examples=["numeric"])
    missing_count: int = Field(..., description="Missing count of values", examples=[0])
    variance: float = Field(..., description="Calculated sample variance of column", examples=[45.2])
    is_constant: bool = Field(..., description="True if column is constant or near constant", examples=[False])
...
class FeatureEngineeringResult(BaseModel):
    ...
    encoding_report: EncodingReport = Field(..., description="Summary details of encodings")
    scaling_report: ScalingReport = Field(..., description="Summary details of scalings")
    selection_report: SelectionReport = Field(..., description="Summary details of selections")
    leakage_report: LeakageReport = Field(..., description="Summary details of leakage checks")
    split_report: SplitReport = Field(..., description="Summary details of splits")
    pipeline_report: PipelineReport = Field(..., description="Summary details of serialized pipelines")
    train_filepath: str = Field(..., description="Physical path of training segment", examples=["/app/workspace/processed/train.csv"])
    val_filepath: str = Field(..., description="Physical path of validation segment", examples=["/app/workspace/processed/val.csv"])
    test_filepath: str = Field(..., description="Physical path of test segment", examples=["/app/workspace/processed/test.csv"])
    duration_seconds: float = Field(..., description="Pipeline execution duration in seconds", examples=[3.42])
