...
class ReportConfig(BaseModel):
    ...
    theme_name: str = Field(
        default="corporate",
        description="Layout styling themes ('light', 'dark', 'corporate')",
    )
    include_manifest: bool = Field(
        default=True, description="Whether to generate report_manifest.json index"
    )
    auto_resize_assets: bool = Field(
        default=True, description="Enable automatic graph image scaling"
    )
