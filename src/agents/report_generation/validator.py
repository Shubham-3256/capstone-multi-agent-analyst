...
class ReportValidator:
    ...
    def validate_report(
        ...
        if missing_sections:
            logger.error(
                "ReportValidator: Missing sections: %s", sorted(missing_sections)
            )
            is_valid = False
        ...
        for chart_name in manifest.charts_included:
            ...
            if not chart_file.exists():
                logger.error(
                    f"ReportValidator: Asset reference {chart_file} not found on disk."
                )
                is_valid = False
        ...
        for section in sections.values():
            for figure in section.figures:
                if Path(figure.file_path).name not in manifest_assets:
                    logger.error(
                        "ReportValidator: %s is not represented in the manifest.",
                        figure.file_path,
                    )
                    is_valid = False
        ...
        for sec_id, section in sections.items():
            ...
            if matches:
                logger.warning(
                    f"ReportValidator: Found unresolved template placeholders in '{sec_id}': {matches}"
                )
                is_valid = False
        ...
        if output_paths:
            for output_type, output_path in output_paths.items():
                if (
                    ...
                    logger.error(
                        "ReportValidator: %s export is missing or empty.", output_type
                    )
                    is_valid = False

        if is_valid:
            logger.info(
                "ReportValidator: Document validation checks passed successfully."
            )
        else:
            logger.error(
                "ReportValidator: Document validation failures discovered. See warnings."
            )
