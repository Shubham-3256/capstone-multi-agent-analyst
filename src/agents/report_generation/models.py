...
class FigureReference(BaseModel):
    ...
    file_path: str = Field(
        ..., description="Absolute or relative path to graph image file"
    )
...
class ReportSection(BaseModel):
    ...
    content_markdown: str = Field(
        ..., description="Body section text formatted in markdown"
    )
    figures: list[FigureReference] = Field(
        default_factory=list, description="Embedded figure references list"
    )
    tables: list[TableReference] = Field(
        default_factory=list, description="Embedded data table references list"
    )
...
class ReportContext(BaseModel):
    ...
    dataset_name: str = Field(
        default="Analyst Dataset",
        description="Name descriptor for analyzed data source",
    )
    dataset_profile_str: str = Field(
        ..., description="Overview summaries from Data Intelligence Agent"
    )
    feature_summary_str: str = Field(
        ..., description="Overview summaries from Feature Engineering Agent"
    )
    ml_summary_str: str = Field(
        ..., description="AutoML leaderboard outputs from Machine Learning Agent"
    )
    viz_summary_str: str = Field(
        ..., description="Graph layouts metadata from Visualization Agent"
    )
    insights_str: str = Field(
        ...,
        description="Corporate summaries and recommendations from Business Insight Agent",
    )
    charts_paths: list[str] = Field(
        default_factory=list, description="Paths to generated chart visual assets"
    )
...
class ReportManifest(BaseModel):
    ...
    dataset_hash: str = Field(
        ..., description="SHA256 signature hash of target source data"
    )
    ...
    charts_included: list[str] = Field(
        default_factory=list,
        description="Chart file basenames exported inside document folders",
    )
    sections: list[str] = Field(
        default_factory=list, description="Ordered list of chapter titles generated"
    )
    generation_timestamp: str = Field(
        ..., description="UTC ISO format creation timestamp"
    )
...
class ReportMetadata(BaseModel):
    ...
    template_type: str = Field(
        ..., description="Template model selected ('executive', 'technical', 'audit')"
    )
...
class ReportResult(BaseModel):
    ...
    output_paths: dict[str, str] = Field(
        ...,
        description="Paths mapping exported format keys (markdown, html, pdf, docx)",
    )
    manifest: ReportManifest = Field(
        ..., description="Compiled report manifest index metadata"
    )
