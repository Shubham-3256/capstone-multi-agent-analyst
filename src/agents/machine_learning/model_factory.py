...
class ModelFactory:
    ...
    def get_candidate_models(task_type: str, candidate_names: List[str], random_seed: int = 42) -> Dict[str, Any]:
        ...
        if task_type == "classification":
            # Map of classifiers
            registry = {
                "logistic_regression": lambda: LogisticRegression(random_state=random_seed, max_iter=1000),
                "decision_tree": lambda: DecisionTreeClassifier(random_state=random_seed),
            ...
            try:
                ...
                registry["xgboost"] = lambda: XGBClassifier(random_state=random_seed, eval_metric="logloss")
            except ImportError:
                pass

            try:
                from lightgbm import LGBMClassifier
                registry["lightgbm"] = lambda: LGBMClassifier(random_state=random_seed, verbose=-1)
            except ImportError:
                pass
            ...
            registry = {
                ...
                "decision_tree": lambda: DecisionTreeRegressor(random_state=random_seed),
                "random_forest": lambda: RandomForestRegressor(random_state=random_seed),
            ...
            try:
                ...
                registry["lightgbm"] = lambda: LGBMRegressor(random_state=random_seed, verbose=-1)
            except ImportError:
                pass
        ...
        for name in candidate_names:
            ...
            if key in registry:
                try:
                    ...
                    logger.warning(f"ModelFactory: Failed to instantiate '{name}': {e}. Skipping.")
            else:
                # Graceful warning for unsupported candidates
                logger.warning(f"ModelFactory: Model name '{name}' is unsupported or missing dependencies. Skipping.")
