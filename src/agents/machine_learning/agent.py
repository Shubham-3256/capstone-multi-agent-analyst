        ...
        try:
            ...
            if not candidates:
                raise DatasetException(
                    "No candidate models could be instantiated by the ModelFactory."
                )
            ...
            for name, model in candidates.items():
                logger.info(
                    f"MachineLearningAgent: Processing model candidate '{name}'..."
                )
                ...
                if fit_result.error_message:
                    logger.warning(
                        f"Skipping model evaluation for '{name}' due to fit error."
                    )
                    continue
            ...
            leaderboard, best_model_name = ModelRanker.rank_models(
                candidate_metrics, task_type
            )
            ...
            feature_importances = Explainer.explain_model(
                best_model, validation_data, validation_target, task_type
            )
            ...
            logger.info(
                f"MachineLearningAgent: Modeling sweep finished successfully in {round(duration, 4)}s"
            )
            ...
            self.db.commit()
            raise DatasetException(
                f"Pipeline error executing MachineLearningAgent: {e}"
            ) from e
