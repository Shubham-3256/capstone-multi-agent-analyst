...
class TaskReport(BaseModel):
    ...
    classes: Optional[List[Any]] = Field(default=None, description="Unique values of target classes if classification", examples=[[0, 1]])
    is_binary: Optional[bool] = Field(default=None, description="True if classification target contains exactly 2 unique values", examples=[True])
...
class TrainingResult(BaseModel):
    ...
    error_message: Optional[str] = Field(default=None, description="Captured model training errors", examples=[None])
...
class EvaluationReport(BaseModel):
    ...
    residuals_summary: Optional[Dict[str, float]] = Field(
        default=None,
