...
class ExecutiveSummary(BaseModel):
    ...
    headline: str = Field(
        ..., description="High-level punchy business summary statement"
    )
    key_takeaways: list[str] = Field(
        ..., description="List of primary operational summary points"
    )
    impact_statement: str = Field(
        ..., description="Details regarding business value or cost reductions"
    )
...
class DatasetInsight(BaseModel):
    ...
    completeness_score: float = Field(
        ..., description="Value representing completeness ratios"
    )
    anomalies_detected: list[str] = Field(
        ..., description="Issues discovered in data schema formats"
    )
    recommendation: str = Field(
        ..., description="Suggested operations cleaning actions"
    )
...
class ModelInsight(BaseModel):
    ...
    feature_weights: dict[str, float] = Field(
        ..., description="Coefficients representing feature importances"
    )
    conclusion: str = Field(
        ..., description="Summary explanation of model validity and error bounds"
    )
...
class Recommendation(BaseModel):
    ...
    actionability: str = Field(
        ..., description="Actionability rating index ('High', 'Medium', 'Low')"
    )
...
class RiskItem(BaseModel):
    ...
    probability: str = Field(
        ..., description="Likelihood index ('High', 'Medium', 'Low')"
    )
...
class ConfidenceReport(BaseModel):
    ...
    confidence_score: float = Field(
        ..., description="Audit confidence score between 0.0 and 1.0"
    )
    reliability_rating: str = Field(
        ..., description="Reliability rating ('High', 'Medium', 'Low')"
    )
...
class BusinessInsightResult(BaseModel):
    ...
    executive_summary: ExecutiveSummary = Field(
        ..., description="Executive summary overview findings"
    )
    dataset_insight: DatasetInsight = Field(
        ..., description="Data quality validation metrics findings"
    )
    model_insight: ModelInsight = Field(
        ..., description="Model validation accuracy interpretations"
    )
    recommendations: list[Recommendation] = Field(
        default=[], description="Strategic recommendations checklist"
    )
    risks: list[RiskItem] = Field(
        default=[], description="Identified operational risk items"
    )
    confidence_report: ConfidenceReport = Field(
        ..., description="Analysis confidence verification report"
    )
    token_usage: dict[str, float] = Field(
        default_factory=dict, description="Estimated token usage summary count"
    )
