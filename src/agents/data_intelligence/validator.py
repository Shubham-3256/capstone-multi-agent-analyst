...
class Validator:
    ...
    @staticmethod
    def validate(
        df: pd.DataFrame, file_path: Path, target_column: str | None = None
    ) -> ValidationReport:
        ...
        if rows == 0 or cols == 0:
            ...
            return ValidationReport(
                is_valid=is_valid,
                issues=issues,
                summary={"total_issues": len(issues), "errors": 1, "warnings": 0},
            )
        ...
        invalid_col_pattern = re.compile(
            r"[^\w\-]"
        )  # Non-alphanumeric/underscore/dash characters
        ...
        logger.info(
            f"Validator: Ingestion checked. Valid: {is_valid}. Errors: {errors}, Warnings: {warnings}"
        )
        return ValidationReport(is_valid=is_valid, issues=issues, summary=summary)
