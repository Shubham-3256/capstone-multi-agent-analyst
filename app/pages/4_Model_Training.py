...
def main() -> None:
    ...
    if not ml_result:
        ...
        if not target_col:
            st.warning(
                "AutoML model training was skipped because no target column was selected."
            )
        else:
    ...
    split_rep = getattr(
        getattr(result.state, "feature_result", None), "split_report", None
    )
    train_shape = getattr(split_rep, "train_shape", None)
    if train_shape and train_shape[0] < 10:
        st.warning(
            "⚠️ Results for demonstration only: The dataset is too small (fewer than 10 training samples) for robust model training."
        )
    ...
    if ml_result.feature_importances:
        ...
        st.markdown(
            "Feature importances calculated from the best fitted AutoML estimator:"
        )
