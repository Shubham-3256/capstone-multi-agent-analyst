    ...
    try:
        ...
        if state:
            result = WorkflowResult(
                ...
                state=state,
                output_paths=dict(
                    getattr(state.report_result, "output_paths", {}) or {}
                ),
            )
...
def main() -> None:
    ...
    with col_c1:
        status_filter = st.selectbox(
            "Filter by Status", options=["All", "Completed", "Degraded"], index=0
        )
    with col_c2:
        search_query = st.text_input(
            "Search by ID or keywords", value="", placeholder="Search workflow ID..."
        )
    ...
    st.markdown(
        "Select a workflow ID from the dropdown below to load its full state back into the active session."
    )

    workflow_ids_list = [w.workflow_id for w in workflows]
    selected_id = st.selectbox(
        "Select Workflow ID to Inspect", options=workflow_ids_list
    )

    if st.button("🔄 Inspect and Reload Selected Run", width="stretch"):
        if load_past_workflow_result(selected_id):
            st.success(
                f"🎉 **Workflow ID `{selected_id}` successfully loaded!** Navigate to pages 2-7 to inspect its data."
            )
            st.toast("Past workflow loaded successfully!")
        else:
            st.error(
                "Could not load the state checkpoint for this workflow ID (checkpoint file might be missing or corrupted)."
            )
