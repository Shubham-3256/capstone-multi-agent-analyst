...
class LiveProgressViewer:
    ...
    def __call__(self, event: Any) -> None:
        ...
        node_title = (
            event.node_name.replace("_", " ").title() if event.node_name else ""
        )
        ...
        with self.placeholder.container():
            ...
            with st.status(
                f"⚙️ **{status_text}**", expanded=True, state=state_str
            ) as status_box:
                for ev in self.events:
                    ev_node = (
                        ev.node_name.replace("_", " ").title() if ev.node_name else ""
                    )
                    if ev.event_type == "node_started":
                        ...
                        st.markdown(
                            f"❌ **Failed** `{ev_node}`: {ev.payload.get('error', 'Unknown Error')}"
                        )
                ...
                if event.event_type == "workflow_completed":
                    if event.payload.get("success"):
                        ...
                        status_box.update(
                            label="⚠️ **Workflow Completed with Errors.**",
                            state="error",
                        )
    ...
    for idx, node in enumerate(all_possible_nodes, 1):
        ...
        if node in executed_nodes:
            ...
            if item.status == "completed":
                st.success(
                    f"✅ **{node_title}** — Completed in {item.duration_seconds:.2f}s (Attempts: {item.retries + 1})"
                )
            else:
            ...
            target_col = getattr(
                getattr(state, "metadata", None), "target_column", None
            )
