...
def workflow_metrics(result: Any) -> dict[str, float]:
    ...
    return {"nodes": float(len(history)), "seconds": round(sum(timing.values()), 3)}
...
def render_ml_metrics(ml_result: Any) -> None:
    ...
    with col2:
        best_score = (
            ml_result.best_metrics.get("f1")
            or ml_result.best_metrics.get("rmse")
            or 0.0
        )
    ...
    for entry in leaderboard.entries:
        ...
        rows.append(
            [str(entry.rank), entry.model_name, f"{entry.score:.4f}", metrics_str]
        )
