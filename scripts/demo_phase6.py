...
def run_machine_learning_demo() -> None:
    ...
    logger.info(
        f"Loaded engineered splits. Train shape: {X_train.shape}, Val shape: {X_val.shape}"
    )
    ...
    for entry in result.leaderboard.entries:
        print(
            f"  Rank {entry.rank}: {entry.model_name.ljust(22)} | Score: {round(entry.score, 4)} | Metrics: {entry.metrics}"
        )
    print("=" * 60)
    ...
    for idx, item in enumerate(result.feature_importances[:5]):
        print(
            f"  * Rank {idx + 1}: {item.column.ljust(18)} Importance: {item.importance}"
        )
    ...
    probabilities = (
        loaded_best_model.predict_proba(sample_row)[0]
