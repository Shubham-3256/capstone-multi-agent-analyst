...
def main() -> None:
    ...
    mock_profile = SimpleNamespace(
        ...
        columns={
            "age": SimpleNamespace(
                ...
                null_count=0,
                null_percentage=0.0,
                numeric_summary={"mean": 38.5, "min": 18.0, "max": 90.0},
                categorical_summary=None,
                date_summary=None,
            ),
            "churn": SimpleNamespace(
                name="churn",
                dtype="int64",
                unique_count=2,
                null_count=0,
                ...
                categorical_summary={"top": 0, "freq": 80},
                date_summary=None,
            ),
        },
        correlation_matrix={
            "age": {"age": 1.0, "monthly_charges": 0.42},
            "monthly_charges": {"age": 0.42, "monthly_charges": 1.0},
        },
        ...
        recommendations=[
            SimpleNamespace(
                ...
                description="Stratified splits recommended.",
                severity="warning",
                column="churn",
            )
        ],
    ...
    mock_di = SimpleNamespace(
        ...
        validation_report=SimpleNamespace(
            is_valid=True, issues=[], summary={"total_issues": 0}
        ),
    ...
    mock_fe = SimpleNamespace(
        ...
        scaling_report=SimpleNamespace(
            scaler_type={"age": "standard"},
            scaling_parameters={"age": {"mean": 38.5, "scale": 12.3}},
        ),
        selection_report=SimpleNamespace(
            method="mutual_info",
            original_count=3,
            selected_count=2,
            selected_features=["age", "monthly_charges"],
            feature_importances={"age": 0.42, "monthly_charges": 0.35},
        ),
        leakage_report=SimpleNamespace(has_leakage=False, leakage_issues=[]),
        split_report=SimpleNamespace(
            train_shape=[70, 3],
            val_shape=[15, 3],
            test_shape=[15, 3],
            strategy="stratified",
        ),
        pipeline_report=SimpleNamespace(
            pipeline_filepath="workspace/models/pipeline.joblib",
            components=["scaling", "selection"],
        ),
            ...
            charts=[
                SimpleNamespace(
                    ...
                    caption=SimpleNamespace(
                        summary="Weak correlation",
                        details="No features show significant colinearity.",
                    ),
                ),
            ],
            metadata=SimpleNamespace(
                dataset_hash="mock-hash", created_at="2026-07-18", theme="corporate"
            ),
        ),
    ...
    mock_biz = SimpleNamespace(
        executive_summary=SimpleNamespace(
            headline="High Churn Risk in Age Bracket",
            key_takeaways=["Targeted action recommended", "Ensure database scaling"],
            impact_statement="Reduction in churn by 5%",
        ),
        dataset_insight=SimpleNamespace(
            completeness_score=1.0, anomalies_detected=[], recommendation="Clean"
        ),
        model_insight=SimpleNamespace(
            algorithm_name="RandomForestClassifier",
            accuracy=0.85,
            f1=0.83,
            feature_weights={"age": 0.6},
            conclusion="Valid model",
        ),
        recommendations=[
            SimpleNamespace(
                title="Promotions",
                description="Send discount codes",
                actionability="high",
            )
        ],
        risks=[
            SimpleNamespace(
                severity="medium",
                probability="low",
                description="Overfitting potential",
            )
        ],
        confidence_report=SimpleNamespace(
            confidence_score=0.9,
            reliability_rating="high",
            justification="High validation accuracy",
        ),
    ...
    mock_rep = SimpleNamespace(
        is_success=True,
        output_paths={
            "pdf": "workspace/reports/report.pdf",
            "markdown": "workspace/reports/report.md",
        },
        manifest=SimpleNamespace(
            ...
            generation_timestamp="2026-07-18",
        ),
        metadata=SimpleNamespace(
            title="Customer Churn Analysis",
            author="Multi-Agent Platform",
    ...
    print(
        ...
        [item.status for item in result.state.execution_history],
    )
    print("Persisted timing info:", result.state.timing)
    print(
        "Checkpoint Saved path:",
        workflow.file_checkpoints.directory
        / f"{result.state.metadata.workflow_id}.json",
    )
