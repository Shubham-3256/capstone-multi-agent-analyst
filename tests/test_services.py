...
def test_metadata_service_extraction():
    ...
    df = pd.DataFrame({
        ...
        "joined": pd.to_datetime(["2020-01-01", "2021-06-15", "2019-11-23", "2020-01-01"])
    })
...
from app.services.session import initialize_session, set_workflow_result, get_workflow_result, set_uploaded_dataset_path, get_uploaded_dataset_path, clear_session
...
def test_workflow_service_run():
    ...
    mock_graph.run.assert_called_once_with(dataset_path="dummy.csv", target_column="churn")


def test_history_service_queries(db_session):
    """Test that HistoryService correctly queries records from the DB session."""
    from src.database.models import WorkflowExecution, ReportRecord, DatasetRecord
    ...
    with patch("src.database.database.DatabaseManager.get_session", return_value=db_session):
