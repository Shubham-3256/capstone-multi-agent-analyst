...
def test_caption_generator_correlation():
    ...
    assert (
        "linear association" in caption.summary
        or "weak correlations" in caption.summary
    )
...
def test_caption_generator_leaderboard():
    ...
    leaderboard = Leaderboard(
        entries=[
            LeaderboardEntry(
                model_name="RandomForest", rank=1, score=0.91, metrics={"f1": 0.91}
            )
        ]
    )
    caption = CaptionGenerator.generate_leaderboard_caption(
