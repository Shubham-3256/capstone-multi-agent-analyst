...
def clean_db():
    ...
    engine = create_engine(f"sqlite:///{db_file.resolve()}", connect_args={"check_same_thread": False})
...
def test_concurrent_agent_runs(clean_db):
    ...
    def worker_run(idx):
        ...
        thread_local_paths.processed_dir = Paths._ORIGINAL_PROCESSED_DIR.parent / f"processed_thread_{idx}"
