...
def generate_titanic_schema() -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "Cabin": ["C85", "C123", np.nan, np.nan, "E46", np.nan, np.nan, "G6", "C103", np.nan] * 10,
...
def generate_california_housing_schema() -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "AveRooms": [6.98, 6.23, 8.28, 5.81, 6.28, 4.76, 5.11, 5.98, 6.32, 5.02] * 10,
        "AveBedrms": [1.02, 0.97, 1.07, 1.07, 1.08, 1.10, 0.95, 1.05, 1.12, 1.00] * 10,
        "Population": [322, 2401, 496, 558, 565, 413, 1094, 1157, 432, 742] * 10,
        "AveOccup": [2.55, 2.10, 2.80, 2.54, 2.18, 2.45, 1.80, 2.20, 2.40, 2.30] * 10,
        ...
        "MedHouseValue": [452600.0, 358500.0, 352100.0, 341300.0, 342200.0, 269700.0, 299200.0, 241400.0, 226700.0, 261100.0] * 10 + np.random.randn(100) * 1000.0
    })
...
def generate_adult_income_schema() -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "workclass": ["State-gov", "Self-emp-not-inc", "Private", "Private", "Private", "Private", "Private", "Self-emp-not-inc", "Private", "Private"] * 10,
        "education": ["Bachelors", "Bachelors", "HS-grad", "11th", "Bachelors", "Masters", "9th", "HS-grad", "Masters", "Bachelors"] * 10,
        "marital-status": ["Never-married", "Married-civ-spouse", "Divorced", "Married-civ-spouse", "Married-civ-spouse", "Married-civ-spouse", "Married-spouse-absent", "Married-civ-spouse", "Never-married", "Married-civ-spouse"] * 10,
        "occupation": ["Adm-clerical", "Exec-managerial", "Handlers-cleaners", "Handlers-cleaners", "Prof-specialty", "Exec-managerial", "Other-service", "Exec-managerial", "Prof-specialty", "Exec-managerial"] * 10,
        "race": ["White", "White", "White", "Black", "Black", "White", "Black", "White", "White", "White"] * 10,
        "gender": ["Male", "Male", "Male", "Male", "Female", "Female", "Female", "Male", "Female", "Male"] * 10,
        "hours-per-week": [40, 13, 40, 40, 40, 40, 16, 45, 50, 40] * 10,
        "income": ["<=50K", ">50K", "<=50K", "<=50K", "<=50K", ">50K", "<=50K", ">50K", ">50K", ">50K"] * 10
    })
...
def run_pipeline_on_dataset(df, target_col, clean_db, filename):
    ...
    graph = WorkflowGraph(config=WorkflowConfig(checkpoint_mode="none", persist_execution=False))
    graph.db = clean_db
