...
def test_dataset_record_operations(clean_db):
    ...
    retrieved = (
        clean_db.query(DatasetRecord).filter_by(file_hash="uniquehash12345").first()
    )
...
def test_report_and_workflow_execution_records(clean_db):
    ...
    assert (
        clean_db.query(ReportRecord).filter_by(report_id="rep-111").first() is not None
    )
    assert (
        clean_db.query(WorkflowExecution).filter_by(workflow_id="wf-111").first()
        is not None
    )
