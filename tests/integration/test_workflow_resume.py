...
def test_workflow_checkpoint_and_resume(clean_db):
    ...
    wf_record = clean_db.query(WorkflowExecution).filter_by(workflow_id=workflow_uuid).first()
    assert wf_record is not None
    
    # Reload and run workflow to completion starting from the state checkpoint
    result = resumed_graph.run(str(dataset_file), target_column="label", workflow_id=workflow_uuid)
    ...
    updated_rec = clean_db.query(WorkflowExecution).filter_by(workflow_id=workflow_uuid).first()
